...
class YOLO(object):
    ...
    def custom_loss(self, y_true, y_pred):
        ...
        if self.debug:
            ...
            nb_pred_box = tf.reduce_sum(
                tf.to_float(true_box_conf > 0.5) * tf.to_float(
                    pred_box_conf > 0.3))
    ...
    def predict(self, image):
        ...
        dummy_array = dummy_array = np.zeros((1, 1, 1, 1,
    ...
    def train(
        ...
        self.warmup_bs = warmup_epochs * (
            train_times * (len(train_imgs) / batch_size + 1) + valid_times *
        ...
        train_batch = BatchGenerator(
            train_imgs, generator_config, norm=self.feature_extractor.normalize)
